...
describe('GasCar', () => {
    ...
    it('constructor sets basic properties and initial energy level', () => {
        expect(car.brand).to.equal(brand);
        expect(car.model).to.equal(model);
        expect(car.maxSpeed).to.equal(maxSpeed);
        expect(car.color).to.equal(color);

        const expectedPercent = Math.round((initialFuel / tankCapacity) * 100);
        expect(car.getEnergyLevel()).to.equal(expectedPercent);
    });
    ...
    it('refill increases fuel level and updates capacity (clamped by tank capacity)', () => {
        ...
        expect(car.getEnergyLevel()).to.equal(expectedAfterRefill);
        car.refill(1000);
        expect(car.getEnergyLevel()).to.equal(100);
        car.refill(-1000);
        expect(car.getEnergyLevel()).to.equal(0);
    });

    it('getEnergyLevel returns current capacity percent', () => {
        expect(car.getEnergyLevel()).to.equal(Math.round((initialFuel / tankCapacity) * 100));
        car.refill(5);
        expect(car.getEnergyLevel()).to.equal(Math.round((Math.min(initialFuel + 5, tankCapacity) / tankCapacity) * 100));
    });
});
